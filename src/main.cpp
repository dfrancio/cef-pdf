...
void printHelp(std::string name)
{
    ...
    std::cout << "                   " << cefpdf::GetDefaultPageSizeName() << " is the default." << std::endl;
    ...
    std::cout << "  --port=<port>    Specify server port number. Default is " << cefpdf::GetDefaultServerPort() << std::endl;
    std::cout << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    if (cefpdf::HasSwitch(commandLine, "javascript")) {
        EnableJavaScript(app);
    } else {
        DisableJavaScript(app);
    }
