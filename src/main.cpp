...
void printSizes() {
  ...
  for (it = cefpdf::pageSizesMap.begin(); it != cefpdf::pageSizesMap.end();
       ++it) {
...
void printHelp(std::string name) {
  ...
  std::cout << "  cef-pdf [options] --url=<url>|--file=<path> [output]"
            << std::endl;
  std::cout << std::endl;
  std::cout << "Options:" << std::endl;
  std::cout << "  --help -h         This help screen." << std::endl;
  std::cout << "  --url=<url>       URL to load, may be http, file, data, "
               "anything supported by Chromium."
            << std::endl;
  std::cout << "  --file=<path>     File path to load using file:// scheme. "
               "May be relative to current directory."
            << std::endl;
  std::cout << "  --stdin           Read content from standard input until EOF "
               "(Unix: Ctrl+D, Windows: Ctrl+Z)."
            << std::endl;
  std::cout << "  --size=<spec>     Size (format) of the paper: A3, B2.. or "
               "custom <width>x<height> in mm."
            << std::endl;
  std::cout << "                    " << cefpdf::constants::pageSize
            << " is the default." << std::endl;
  std::cout << "  --list-sizes      Show all defined page sizes." << std::endl;
  std::cout << "  --landscape       Wheather to print with a landscape page "
               "orientation."
            << std::endl;
  std::cout << "                    Default is portrait." << std::endl;
  std::cout << "  --margin=<spec>   Paper margins in mm (much like CSS margin "
               "but without units)"
            << std::endl;
  std::cout << "                    If omitted some default margin is applied."
            << std::endl;
  std::cout << "  --javascript      Enable JavaScript." << std::endl;
  std::cout << "  --backgrounds     Print with backgrounds. Default is without."
            << std::endl;
  std::cout << "  --scale=<%>       Scale the output. Default is 100."
            << std::endl;
  std::cout << "  --delay=<ms>      Wait after page load before creating PDF. "
               "Default is 0."
            << std::endl;
  std::cout << "  --viewwidth=<px>  Width of viewport. Default is 128."
            << std::endl;
  std::cout << "  --viewheight=<px> Height of viewport. Default is 128."
            << std::endl;
  std::cout << std::endl;
  std::cout << "Server options:" << std::endl;
  std::cout << "  --server          Start HTTP server" << std::endl;
  std::cout << "  --host=<host>     If starting server, specify ip address to "
               "bind to."
            << std::endl;
  std::cout << "                    Default is "
            << cefpdf::constants::serverHost << std::endl;
  std::cout << "  --port=<port>     Specify server port number. Default is "
            << cefpdf::constants::serverPort << std::endl;
  std::cout << std::endl;
  std::cout << "Output:" << std::endl;
  std::cout << "  PDF file name to create. Default is to write binary data to "
               "standard output."
            << std::endl;
  std::cout << std::endl;
}
...
int runJob(CefRefPtr<cefpdf::Client> app,
  ...
  try {
    if (commandLine->HasSwitch("stdin")) {
      ...
      if (commandLine->HasSwitch("url")) {
        ...
        // removed verification of existing file because always will be
        // /usr/local/share/fmpcore/report/Default.html.
        // With this protection we can not pass others parameters to generate
        // pdf file we can not pass others parameters to generate pdf file
        //
        //
    ...
    if (commandLine->HasSwitch("scale")) {
      job->SetScale(
          std::atoi(commandLine->GetSwitchValue("scale").ToString().c_str()));
    }

    if (commandLine->HasSwitch("delay")) {
      app->SetDelay(
          std::atoi(commandLine->GetSwitchValue("delay").ToString().c_str()));
    }

    if (commandLine->HasSwitch("viewwidth")) {
      app->SetViewWidth(std::atoi(
          commandLine->GetSwitchValue("viewwidth").ToString().c_str()));
    }

    if (commandLine->HasSwitch("viewheight")) {
      app->SetViewHeight(std::atoi(
          commandLine->GetSwitchValue("viewheight").ToString().c_str()));
    }
  ...
  if (readFromStdIn && !writeToStdOut) {
    std::cout << "Waiting for input until EOF (Unix: Ctrl+D, Windows: Ctrl+Z)"
              << std::endl;
  }
...
int runServer(CefRefPtr<cefpdf::Client> app,
              CefRefPtr<CefCommandLine> commandLine) {
  ...
  CefRefPtr<cefpdf::server::Server> server =
      new cefpdf::server::Server(app, host, port);
...
int main(int argc, char *argv[]) {
  ...
  return commandLine->HasSwitch("server") ? runServer(app, commandLine)
                                          : runJob(app, commandLine);
}
