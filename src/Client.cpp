...
Client::Client()
    : m_jobManager(new job::Manager()), m_pendingBrowsersCount(0),
      m_browsersCount(0), m_initialized(false), m_contextInitialized(false),
      m_running(false), m_stopAfterLastJob(false),
      m_printHandler(new PrintHandler), m_renderHandler(new RenderHandler),
  ...
  CefString(&m_browserSettings.default_encoding)
      .FromString(constants::encoding);
  ...
  m_browserSettings.universal_access_from_file_urls = STATE_ENABLED;
  m_delay = 0;
}
...
void Client::OnBeforeCommandLineProcessing(
    const CefString &process_type, CefRefPtr<CefCommandLine> command_line) {
  DLOG(INFO) << "Client::OnBeforeCommandLineProcessing"
...
// -----------------------------------------------------------------------------
CefRefPtr<CefPrintHandler> Client::GetPrintHandler() { return m_printHandler; }

void Client::OnBeforeChildProcessLaunch(
    CefRefPtr<CefCommandLine> command_line) {
  DLOG(INFO) << "Client::OnBeforeChildProcessLaunch"
             << " with command_line: "
             << command_line->GetCommandLineString().ToString();
}
...
void Client::OnContextInitialized() {
  ...
  CefRegisterSchemeHandlerFactory(constants::scheme, "",
                                  new SchemeHandlerFactory(m_jobManager));
...
void Client::OnLoadEnd(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame,
  ...
  if (frame->IsMain()) {
    if (httpStatusCode == 200 && m_delay > 0) {
      DLOG(INFO) << "Client::OnLoadEnd - waiting for " << m_delay
                 << "ms before generating PDF";
      CefPostDelayedTask(TID_UI, base::Bind(&Client::Process, this, browser),
                         m_delay);
    } else
